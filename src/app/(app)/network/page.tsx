    ...
    <div className="px-4 pt-6 max-w-lg mx-auto space-y-8">
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-semibold text-sm" style={{ color: "var(--text)" }}>Streak de Amigos</h2>
        ...
        {friends.length > 0 ? (
            ...
            {friends.map((friend) => (
              ...
              >
                ...
                <div className="flex items-center gap-1">
                  <Flame className="w-3.5 h-3.5" style={{ color: friend.best_streak > 0 ? "var(--forest)" : "var(--text-muted)" }} />
                  <span className="font-mono text-xs" style={{ color: friend.best_streak > 0 ? "var(--forest)" : "var(--text-muted)" }}>
                    {friend.best_streak}
                  </span>
                </div>
              </div>
          ...
          <div className="rounded-xl p-6 text-center" style={{ background: "var(--surface)", border: "1px solid var(--border-subtle)" }}>
      ...
      <section>
        <h2 className="font-semibold text-sm mb-3" style={{ color: "var(--text)" }}>Meus Apoiadores</h2>

        {goals.length === 0 ? (
          <p className="text-sm" style={{ color: "var(--text-muted)" }}>Crie uma meta primeiro.</p>
        ) : (
          <div className="space-y-4">
            {goalSupporters.map(({ goal, supporters }) => (
              <div key={goal.id} className="rounded-xl p-4" style={{ background: "var(--surface)", border: "1px solid var(--border-subtle)" }}>
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-medium" style={{ color: "var(--text)" }}>{goal.title}</h3>
      ...
      {goalsISupport.length > 0 && (
        <section>
          <h2 className="font-semibold text-sm mb-3" style={{ color: "var(--text)" }}>Metas que Apoio</h2>
          <div className="space-y-2">
            {goalsISupport.map((s) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-medium" style={{ color: "var(--text)" }}>{owner?.name as string}</p>
                    <p className="text-[10px]" style={{ color: "var(--text-muted)" }}>{goal?.title as string}</p>
                  </div>
